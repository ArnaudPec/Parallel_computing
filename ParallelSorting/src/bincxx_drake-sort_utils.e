...
void
check_sorted(char *input_filename, cfifo_int_t *fifo)
{
 size_t i;

 array_int_t *reference = pelib_array_int_loadfilenamebinary(input_filename);


 if(pelib_cfifo_int_length(fifo) != pelib_array_int_length(reference))
 {
  fprintf(stderr, "[%s:%s:%d] Different length of sorted (%zu) and reference arrays (%zu).\n", "utils.c", __FUNCTION__, 23, pelib_cfifo_int_length(fifo), pelib_array_int_length(reference));
  exit(1);
 }
 else
 {
  fprintf(stderr, "[%s:%s:%d] Input length OK.\n", "utils.c", __FUNCTION__, 28);
  int sort_ok = 1;



  qsort((char*)reference->data, pelib_array_int_length(reference), sizeof(int), greater);

  for(i = 0; i < pelib_cfifo_int_length(fifo); i++)
  {
   if(reference->data[i] != fifo->buffer[i])
   {
    sort_ok = 0;
    fprintf(stderr, "[%s:%s:%d] Different value at index %zu. Got %d, expected %d.\n", "utils.c", __FUNCTION__, 40, i, fifo->buffer[i], reference->data[i]);
   }
  }

  if(sort_ok)
  {
   fprintf(stderr, "[%s:%s:%d] No difference between sorted and qsorted arrays.\n", "utils.c", __FUNCTION__, 46);
  }
  else
  {
   exit(1);
...
enable_backtraces()
{
        ...
        sa.__sigaction_handler.sa_sigaction = bt_sighandler;
        sigemptyset (&sa.sa_mask);
        sa.sa_flags = 0x10000000 | 4;


        sigaction(11, &sa, ((void *)0));

        sigaction(8, &sa, ((void *)0));

        sigaction(9, &sa, ((void *)0));
