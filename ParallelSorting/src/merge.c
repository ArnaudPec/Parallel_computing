...
/*
	...
	while(pelib_cfifo_length(int)(left_fifo)>0 && pelib_cfifo_length(int)(right_fifo)>0
	        && !pelib_cfifo_is_full(size_t)(parent_fifo)){

		int left_val = pelib_cfifo_peek(int)(left_fifo, 0);
		int right_val = pelib_cfifo_peek(int)(right_fifo, 0);
		if (left_val < right_val) {
			pelib_cfifo_push(int)(parent_fifo, pelib_cfifo_pop(int)(left_fifo));
		}
		else {
			pelib_cfifo_push(int)(parent_fifo, pelib_cfifo_pop(int)(right_fifo));
		}

	}
    if(!(right_link->prod != NULL && right_link->prod->status < TASK_KILLED) && pelib_cfifo_length(int)(right_fifo) == 0 )
	    while(pelib_cfifo_length(int)(left_fifo)>0 && !pelib_cfifo_is_full(size_t)(parent_fifo))
    {
        pelib_cfifo_pop(int)(left_fifo);
    }
    if(!(left_link->prod != NULL && left_link->prod->status < TASK_KILLED) && pelib_cfifo_length(int)(left_fifo) == 0)
        while(pelib_cfifo_length(int)(right_fifo)>0 && !pelib_cfifo_is_full(size_t)(parent_fifo))
    {
        pelib_cfifo_pop(int)(right_fifo);
    }
...
*/
	...
	//     int var = pelib_cfifo_peek(int)(fifo, 0)
