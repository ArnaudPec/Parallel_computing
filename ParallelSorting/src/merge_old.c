...
drake_run(task_t *task)
{
    ...
    if((left_link->prod == NULL || left_link->prod->status >= TASK_KILLED)
    ...
    else if((left_link->prod == NULL) || left_link->prod->status >= TASK_KILLED){
        printf("%s\n%zu %s", "left job is finished", right_available_elements, " remaining right");
    ...
    else if((right_link->prod == NULL) || right_link->prod->status >= TASK_KILLED){
        printf("%s\n%zu %s", "right job is finished", left_available_elements, " remaining left");
        ...
        do{
            printf("loop\n");
            r_var = pelib_cfifo_peek(int)(right_fifo, 0);
            l_var = pelib_cfifo_peek(int)(left_fifo, 0);
            if (r_var == 0 || l_var == 0 ){
                break;
            }
            else if(r_var<l_var){
                var = pelib_cfifo_pop(int)(right_fifo);
                printf("right popped! %i\n", var);
            }
            else {
                var = pelib_cfifo_pop(int)(left_fifo);
                printf("left popped! %i\n", var);
            }
            pelib_cfifo_push(int)(parent_fifo, var);
  ...
  /*  while(left_available_elements >=0 || right_available_elements >= 0){ 
        int r_var, l_var;
        printf("right %zu\nleft %zu\n", right_available_elements, left_available_elements);

        r_var = pelib_cfifo_peek(int)(right_fifo, 0);
        l_var = pelib_cfifo_peek(int)(left_fifo, 0);
        printf("%i , %i\n", r_var, l_var);
        if(r_var == 0 || l_var ==0){
            printf("%s\n", "smthing is equal to 0"); 
            if(r_var == 0 ){ 
                if(right_link->prod == NULL || left_link->prod->status >= TASK_KILLED){
                    ...
                    right_available_elements = pelib_cfifo_length(int)(right_fifo);
                    //return 0;
                    break;
                }
            } 
            else if(l_var == 0 ){
                if(left_link->prod == NULL || right_link->prod->status >= TASK_KILLED){
                    ...
                    left_available_elements = pelib_cfifo_length(int)(left_fifo);
                  //  return 0;
                    break;
                }
            }
